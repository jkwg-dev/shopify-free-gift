...
- Pricing/enforcement use the Admin API + native discounts only — **NO Shopify Functions are shipped.** (The old "Functions require Plus" premise was disproven — Cart & Checkout **Validation** Functions ARE available on **Advanced** with native error display, no Plus — but the **JS function build does not work in this pnpm + Shopify-CLI 3.91 monorepo**: the CLI demands an explicit wasm build command + shells out to `npm`, and a self-built Javy lacks the Shopify `ShopifyFunction` host glue. So a prototyped validation guard (VF-1) was **SHELVED, not built** — do NOT re-add a Function extension to `extensions/` or `shopify app deploy` breaks for ALL extensions. The drop-below-threshold race is mitigated by the **native BXGY discount minimum** — Shopify reverts a lingering gift line to full price below the tier minimum, so **no revenue leak** (empirically confirmed) — plus the storefront widget's reconciliation; **accepted residual:** an express-checkout (Shop Pay/etc.) lingering gift line is charged, not hard-blocked. "Cumulative suppression needs Plus" rests on the same disproven Functions-premise but still requires a _pricing_ Function, which stays unshipped.) See `docs/checkout-validation-function-design.md` (SHELVED).
