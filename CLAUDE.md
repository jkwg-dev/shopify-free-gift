...
- Pricing/enforcement use the Admin API + native discounts only — **NO discount/pricing Shopify Functions.** A Cart & Checkout **Validation** Function (checkout-time guard, performs **NO pricing** — blocks checkout when a `_fge_gift` line is no longer free) IS shipped, written in **RUST** at `extensions/checkout-validation` (`cart.validations.generate.run`). The JS/TS Function build does NOT work under pnpm + Shopify CLI 3.91 (Javy auto-build doesn't engage; CLI shells out to npm; can't self-build Javy), so Rust — Shopify's first-class Function language, compiling directly to wasm via `cargo` — is a **deliberate, documented exception to "TypeScript everywhere"** (the ONLY non-TS code; isolated — NOT a pnpm/Turborepo package, built only by `cargo` via `[build].command` at deploy; the JS packages are untouched). Verified available on **Advanced** (Functions = all plans except Starter), native error display at checkout, **no Plus / no Checkout UI extension**; runtime-exception behavior via the Checkout Rules **"Allow all customers to submit checkout"** toggle (fail-open lever). **PROD CAVEAT:** the store must be on checkout extensibility (no `checkout.liquid`), and the build machine/CI needs the Rust toolchain (`rustup target add wasm32-unknown-unknown`). (This supersedes the old "NO Functions / require Plus" rule, whose Plus premise was disproven; "cumulative suppression needs Plus" rests on the same disproven premise but still requires a _pricing_ Function, which stays unshipped.) See `docs/checkout-validation-function-design.md`.
...
- **Out-of-stock gift**: if a resolved gift variant is unavailable, `/validate` returns `no-gift` with reason `gift-unavailable` rather than promising an unfulfillable reward; the storefront degrades gracefully.
  - **"Available" spans THREE independent signals (they can diverge):** (a) **priced** in the market (`contextualPricing`), (b) **published** to the Online Store sales channel, (c) **in stock**. `ProductVariant.availableForSale` reflects stock/sellability **not** channel publication, and `contextualPricing` has **no** availability field — so an in-stock, ACTIVE, unpublished gift returns `availableForSale: true` yet **422s at storefront `/cart/add.js`**. Verified live (5b-2a, AND tier). A gift is offerable only if priced **and** published **and** in stock; the `/validate` `gift-unavailable` backstop and the chooser's per-option availability MUST incorporate Online Store **publication** (`Product.publishedOnPublication(<online-store publication>)`), not just `availableForSale`. Reading publication needs **`read_publications`** — `read_products` alone is NOT sufficient (GROUND-TRUTHED: the prod `read_products`-scoped app token returns **`ACCESS_DENIED`** on `publishedOnPublication`; an earlier "works under read_products" probe was actually using a different app token that happened to carry `read_publications`). NOT the deprecated `read_product_listings`. Adding `read_publications` requires merchant **re-consent** (Stage E §5c); until then the channel read degrades to **stock-only** (the pre-E1 reactive-422 gap), never grey-all. Publishing a gift to the channel (provisioning) needs **`write_publications`** + `publishablePublish`. Provisioning must publish gift products; the widget degrades gracefully when add still fails (see `docs/phase-5b-2b-plan.md` + `docs/phase-5b-reseed.md`).
  - **Stage E1 (BUILT — read-only; needs `read_publications` + re-consent):** the publication signal is now wired. One shared pure predicate `giftOfferability({resolved, priced?, publishedToOnlineStore, inStock})` in `packages/core` (reason precedence unresolved → unpriced → not-published → out-of-stock) is the SINGLE source of "is this gift offerable", used by BOTH `/config` (storefront chooser `available` flag) and the `/validate` `gift-unavailable` backstop. Stock + publish are read via `packages/shopify` `fetchGiftChannelAvailability(ids, publicationId)` (`availableForSale` + `product.publishedOnPublication(publicationId:)`, batched 250). **`publishedOnPublication` requires `read_publications`** (now in `GIFT_ENGINE_SCOPES` + `shopify.app.toml` + `SHOPIFY_SCOPES`); if a token lacks it, an `ACCESS_DENIED` on the publication path is classified as **SCOPE_UNAVAILABLE** and the wrapper falls back to a `read_products` stock-only re-read (`publishedToOnlineStore:true`, no gating) — never grey-all — with a loud warn + a boot-time scope check. The reads are resilient via `client.requestPartial` (partial-tolerant; one bad node omits only itself; `request()` stays strict for all other callers). The publication id is `SHOPIFY_ONLINE_STORE_PUBLICATION_ID` (per-environment — dev's `gid://shopify/Publication/157545496685` ≠ prod's), read through `requireOnlineStorePublicationId()` which **fails fast with a named `MissingPublicationConfigError`** if missing/empty/malformed — it NEVER silently falls back to a stock-only check (the FGE_GIFTS_INCLUDED-class trap). For an **AND tier the backstop is all-or-nothing**: any one unavailable required gift → the whole tier yields `gift-unavailable` (one BXGY code grants the AND set together; no partial grant). Availability gates OFFERING only — it is NOT in the minting key, so a filtered option keeps its already-minted code and is re-offered (no re-mint) when it becomes available again. Admin greying (E2) + activate soft-warning / AND-tier note (E3) are designed, not yet built. Design: `docs/phase-3b-stage-e-channel-availability-design.md`.
...
- **Channel/availability scope = `read_publications`** (CORRECTED — `read_products` alone is denied;
  ...
  NOT add the deprecated `read_product_listings`. The policy (don't auto-publish; off-channel → exclude
  from candidates; unlisted → treat as OOS; dropped-off-channel → remove customer-side + dim admin-side
  with explicit removal) is Stage E.
...
- TypeScript strict everywhere. No `any` without an inline justification. (ONE documented exception: `extensions/checkout-validation` is a **Rust** Shopify Function — JS/TS Functions don't build under pnpm + CLI 3.91; it's isolated from the pnpm/Turborepo pipeline and built only by `cargo`. See the hard-constraints note + `docs/checkout-validation-function-design.md`.)
