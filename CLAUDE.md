...
- **Out-of-stock gift**: if a resolved gift variant is unavailable, `/validate` returns `no-gift` with reason `gift-unavailable` rather than promising an unfulfillable reward; the storefront degrades gracefully.
  ...
  - **Stage E1 (BUILT — read-only, no `write_publications`, no reinstall):** the publication signal is now wired. One shared pure predicate `giftOfferability({resolved, priced?, publishedToOnlineStore, inStock})` in `packages/core` (reason precedence unresolved → unpriced → not-published → out-of-stock) is the SINGLE source of "is this gift offerable", used by BOTH `/config` (storefront chooser `available` flag) and the `/validate` `gift-unavailable` backstop. Stock + publish are read via `packages/shopify` `fetchGiftChannelAvailability(ids, publicationId)` (`availableForSale` + `product.publishedOnPublication(publicationId:)`, `read_products` only, batched 250). The publication id is `SHOPIFY_ONLINE_STORE_PUBLICATION_ID` (per-environment — dev's `gid://shopify/Publication/157545496685` ≠ prod's), read through `requireOnlineStorePublicationId()` which **fails fast with a named `MissingPublicationConfigError`** if missing/empty/malformed — it NEVER silently falls back to a stock-only check (the FGE_GIFTS_INCLUDED-class trap). For an **AND tier the backstop is all-or-nothing**: any one unavailable required gift → the whole tier yields `gift-unavailable` (one BXGY code grants the AND set together; no partial grant). Availability gates OFFERING only — it is NOT in the minting key, so a filtered option keeps its already-minted code and is re-offered (no re-mint) when it becomes available again. Admin greying (E2) + activate soft-warning / AND-tier note (E3) are designed, not yet built. Design: `docs/phase-3b-stage-e-channel-availability-design.md`.
