...
- Pricing/enforcement use the Admin API + native discounts only — **NO discount/pricing Shopify Functions.** A Cart & Checkout **Validation** Function MAY be used as a checkout-time guard that performs **NO pricing** (it only blocks checkout when a `_fge_gift` line is no longer free). Verified available on **Advanced** (Shopify Functions = all plans except Starter), with **native error display at checkout** — **no Plus, no Checkout UI extension**; runtime-exception behavior is controlled by the Checkout Rules **"Allow all customers to submit checkout"** toggle (our fail-open lever). **PROD CAVEAT:** the live store must be on **checkout extensibility** (no `checkout.liquid`) for Function APIs to work. (This scoped amendment SUPERSEDES the prior "NO Shopify Functions anywhere / require Plus" rule, whose Plus premise was disproven against docs. The separate "cumulative suppression needs Plus" claim rests on the same disproven premise and should be re-verified before relying on it.) See `docs/checkout-validation-function-design.md`.
